...
"""
Given a .trees file and a string indicating an extant witness or the ancestor of multiple witnesses, 
returns a dictionary mapping its siblings in the sampled trees to their posterior probabilities.
Optionally, a burn-in proportion can be specified.
"""
def get_sibling_distribution(trees_file:Path, target_clade:str, burnin:float=0.0):
    posteriors_by_sibling = {}
            ...
            for line in f:
                ...
                if line.strip().startswith("tree"):
                    ...
                    if len(common_ancestor_witnesses) != len(target_wit_indices):
                        serialized_siblings = "Clade not present"
                        if serialized_siblings not in posteriors_by_sibling:
                            posteriors_by_sibling[serialized_siblings] = 0
                        posteriors_by_sibling[serialized_siblings] += 1
                        trees_processed += 1
                        pbar.update(1)
                        continue
                    # Then proceed through the other children of the common ancestor's parent:
                    siblings = []
                    for sib in common_ancestor.up.children:
                        # Serialize this sibling in terms of the IDs of the leaves under it:
                        sib_str = "(" + ",".join(sorted([ids_by_index[node.name] for node in sib.traverse() if node.name != ""])) + ")"
                        # Skip the common ancestor of the target witnesses:
                        if sib_str == sorted_target_clade:
                            continue
                        siblings.append(sib_str)
                    # Then serialize this and add it to the dictionary:
                    serialized_siblings = str(sorted(siblings))
                    if serialized_siblings not in posteriors_by_sibling:
                        posteriors_by_sibling[serialized_siblings] = 0
                    posteriors_by_sibling[serialized_siblings] += 1
                    trees_processed += 1
                    pbar.update(1)
    # Finally, normalize the frequencies of the siblings:
    for sibling in posteriors_by_sibling:
        posteriors_by_sibling[sibling] = posteriors_by_sibling[sibling] / (trees_processed - burnin_trees)
    return posteriors_by_sibling

"""
Entry point to the script. Parses command-line arguments and calls the core functions.
"""
def main():
    parser = argparse.ArgumentParser(description="Extract a dictionary of posterior probabilities for a given witness's siblings in the sampled trees from a given .trees file.")
    parser.add_argument("trees_input", type=str, help="Input .trees file.")
    parser.add_argument("clade", type=str, help="ID string for the target witness or a string representing the common ancestor of multiple witnesses (e.g., \"(1739,1881)\").")
    ...
    posteriors_by_sibling = get_sibling_distribution(trees_input, clade, burnin)
    for k, v in sorted(posteriors_by_sibling.items(), key=lambda item: item[1], reverse=True):
        if v > 0.001:
            print("%s: %f\n" % (k, v))
    exit(0)
